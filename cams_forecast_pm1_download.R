...
# Parameters
dir_data <- "/media/raphaelsaldanha/lacie/cams_forecast_pm1"
date <- today()
time <- "00:00"
...
# File name
file_name <- glue(
  "cams_forecast_pm1_{substr(date,0,4)}{substr(date,6,7)}{substr(date,9,10)}.nc"
)

# Declare request
request <- list(
  dataset_short_name = "cams-global-atmospheric-composition-forecasts",
  variable = "particulate_matter_1um",
  date = glue("{date}/{date}"),
  time = time,
